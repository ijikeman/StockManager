...
data class DisposeRequest(val unit: Int)
...
class HoldingController(
    ...
    @DeleteMapping("/{lotId}")
    fun deleteHoldingStockLot(@PathVariable lotId: Int) {
        holdingStockService.deleteStockLot(lotId)
    }

    @PostMapping("/{lotId}/dispose")
    fun disposePartialStockLot(
        @PathVariable lotId: Int,
        @RequestBody request: DisposeRequest
    ) {
        holdingStockService.disposePartialStockLot(lotId, request.unit)
    }
}
